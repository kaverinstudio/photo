...
(function ($) {
  ...
  function Galereya(options) {
    ...
    var Handlers = {
      ...
      categoriesListClick: function categoriesListClick(e) {
        var $target = $(e.target),
      ...
      sliderNextClick: function sliderNextClick(e) {
        changeSlide('next');
      },
      sliderPrevClick: function sliderPrevClick(e) {
        changeSlide('prev');
      },
      sliderCloseClick: function sliderCloseClick() {
        closeSlider();
      },
      sliderPlayClick: function sliderPlayClick() {
      ...
      bodyKeyDown: function bodyKeyDown(e) {
        if (isSliderOpened) {
      ...
      bodyKeyUp: function bodyKeyUp(e) {
        if (isSliderOpened) {
          switch (e.which) {
            case 27:
              closeSlider();
    ...
    var getTransitionDuration = function getTransitionDuration($elem) {
      var td = $elem.css('transitionDuration') || $elem.css('webkitTransitionDuration') || $elem.css('mozTransitionDuration') || $elem.css('oTransitionDuration') || $elem.css('msTransitionDuration') || 0;
    ...
    var calcParams = function calcParams() {
      cellW = $cells.width();
    ...
    var createCell = function createCell($img, info) {
      var $cell = $img.addClass('galereya-cell-img').wrap('<div class="galereya-cell" data-index="' + $cells.length + '"></div>').parent().append('<div class="galereya-cell-desc">\
                                <div class="galereya-cell-desc-title">' + info.title + '</div>\
                                <div class="galereya-cell-desc-text">' + info.description + '</div>\
                            </div>').append('<div class="galereya-cell-overlay" />');
    ...
    var addInfo = function addInfo(info) {
      var item = {
        "lowsrc": info.lowsrc || '',
    ...
    var buildDOM = function buildDOM() {
      ...
      $imgs.wrapAll('<div class="galereya-grid" />').each(function (i, img) {
        ...
        $img.addClass('galereya-cell-img').wrap('<div class="galereya-cell" data-index="' + i + '"></div>').parent().append('<div class="galereya-cell-desc">\
      ...
      $slider.addClass(self.options.modifier).append($sliderContainer).append($sliderNext).append($sliderPrev).append($sliderDesc).append($sliderDesc).append($sliderClose).append($sliderPlay);
    ...
    var loadImages = function loadImages(index, category) {
      index = index || 0;
    ...
    var showCell = function showCell(index) {
    ...
    var placeCell = function placeCell(cell, number) {
    ...
    var openSlider = function openSlider(visibleIndex) {
      if (isSliderOpened) {
        setTimeout(function () {
    ...
    var createSlide = function createSlide(visibleIndex) {
      ...
      var cell = visibleCells[visibleIndex],
          ...
          $slide,
          $img;
      $slide = $('<div class="galereya-slider-slide" />').html('<div class="galereya-slide-loader"></div>');
    ...
    var changeSlide = function changeSlide(direction) {
      direction = direction || 'next'; //restore slide show interval, if slide show currently works.
    ...
    var changeSlideState = function changeSlideState($slide, direction) {
      setTimeout(function () {
        $slide.removeClass('prev').removeClass('next').removeClass('current').addClass(direction);
      }, 50);
    };
    /**
     * Update navigation
    ...
    var addEventListeners = function addEventListeners() {
      ...
      $(document.body).click(Handlers.bodyClick).keydown(Handlers.bodyKeyDown).keyup(Handlers.bodyKeyUp);
